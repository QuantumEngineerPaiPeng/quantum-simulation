...
#         return temp
    ...
    def H_pulse_nSpin(self, th):
        temp=1*(np.cos(th)*Hamiltonian(self.nSpin,'p',[1],[1,0,0])+np.sin(th)*Hamiltonian(self.nSpin,'p',[1],[0,1,0]))
        return temp
    
    def step(self, action,stepp,xy,theta):
        pw=self.pw
        e=1/2/np.pi
        ee=0.2
        if action==44:
            ...
            phi=np.pi/2+dis*e
            field=self.H_pulse_nSpin(index*np.pi/2+theta)
        ...
        self.state[24]=stepp/23
        
#         ##### angle imperfection
#         phi=phi+np.pi/200*(np.random.rand(1)[0]*2-1)
        phi=phi+np.pi/200
