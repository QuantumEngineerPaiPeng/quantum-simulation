...
figure(11)
clf;
ycPlot(Y{1},'$n_{period}/2$', '$<Y(n\tau)Y(0)>_{\beta=0}$', ...,
...
ycPlot(fixt_htau, fixt_YY{1}./fixt_YY{1}(1) *XX_list(1) , '$h\tau$', '$<Y(n\tau)Y(0)>_{\beta=0}$', ...,
    ['$h=J, n_{period}=128$'], legends, {{'Marker'}});
ycPlot(fixt_htau, fixt_YY{2}./fixt_YY{2}(1) *XX_list(1), '$h\tau$', '$<Y(n\tau)Y(0)>_{\beta=0}$', ...,
    ['$h=J, n_{period}=128$'], legends, {{'Marker'}});
